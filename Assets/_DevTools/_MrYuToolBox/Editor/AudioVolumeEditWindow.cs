...
public class AudioVolumeEditWindow : EditorWindow
{
    ...
    Dictionary<string, RivetWrong> UIAudioGroup;
    Dictionary<string, RivetWrong> SceneAudioGroup;
    //序列化对象
    ...
    {
        UIAudioGroup = new Dictionary<string, RivetWrong>();
        SceneAudioGroup = new Dictionary<string, RivetWrong>();
        ...
        Dictionary<string, RivetWrong> audioDic = new Dictionary<string, RivetWrong>();
        if (FileInfo.Exists)
        {
        ...
        //UI
        foreach(object music in Enum.GetValues(typeof(ExertMold.UIMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            RivetWrong model = new RivetWrong();
        ...
        // Scene
        foreach (object music in Enum.GetValues(typeof(ExertMold.SceneMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            RivetWrong model = new RivetWrong();
    ...
    public Dictionary<string, RivetWrong> LoadJson(string Filepath)
    {
        StreamReader reader = new StreamReader(Filepath);
        string jsonString = reader.ReadToEnd();
        Dictionary<string, RivetWrong> audioGroup = JsonMapper.ToObject<Dictionary<string, RivetWrong>>(jsonString);
        reader.Dispose();
        reader.Close();
        return audioGroup;
    }
    ...
    {
        Dictionary<string, RivetWrong> dic = new Dictionary<string, RivetWrong>();
        MergeDictionaries.Quest(dic, UIAudioGroup);
        MergeDictionaries.Quest(dic, SceneAudioGroup);
