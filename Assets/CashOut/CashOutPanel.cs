...
public class CashOutPanel : CoaxUIProwl
{
    ...
    private void Start()
    {
        CashOutManager.AirExpertly()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { DelayUIFine(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = PinBeadEka.instance.StatusFive.CashOut_MoneyName;
        InfoText.text = PinBeadEka.instance.StatusFive.CashOut_Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = PinBeadEka.instance.CoaxTug;
        ContactBtn.onClick.AddListener(() => { Application.OpenURL(PinBeadEka.instance.CoaxTug); });
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            OpenPanel(RecordPanel);
            StartLoadingAnim(RecordPanel_Loading);
            CashOutManager.AirExpertly().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.AirExpertly().Account;
        Confirm_AccountText.text = CashOutManager.AirExpertly().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.AirExpertly().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = PinBeadEka.instance.CoaxTug + "/privacy_policy.html";
        ...
        PolicyToggle.isOn = OpenFiveExplain.AirSlay("CashOut_PolicyAgree");
    ...
    {
        base.Hidding();
        GameEvents.CashOutPanelClosedAction?.Invoke();
        CashOutManager.AirExpertly().WaitToSendEvent1304();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.AirExpertly().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.AirExpertly().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.AirExpertly().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            EvokeExplain.AirExpertly().HaleEvoke("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            EvokeExplain.AirExpertly().HaleEvoke(AccountErrorText.text);
            return;
        }

        CashOutManager.AirExpertly().Account = AccountInput.text;
        OpenFiveExplain.BisRecoil("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            EvokeExplain.AirExpertly().HaleEvoke("Please agree to the policy");
            return;
        }
        OpenFiveExplain.BisSlay("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.AirExpertly().Withdraw();
    }
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.AirExpertly().Data.Record == null || CashOutManager.AirExpertly().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.AirExpertly().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.AirExpertly().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            EvokeExplain.AirExpertly().HaleEvoke("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        PorkTruckRevere.AirExpertly().FastTruck("1301", LeftTimeText.text, CashOutManager.AirExpertly().Money.ToString());
    }
}
