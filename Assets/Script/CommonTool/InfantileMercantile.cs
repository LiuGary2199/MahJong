...
public class InfantileMercantile : MonoBehaviour
{
    void Start()
    {
    ...
    public static void LadHale(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void LadTail(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void ParentIonize(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void ParentIonize(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        ParentIonize(startNum, endNum, delay, text, "", finish);
    }
    public static void ParentIonize(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + IonizeSeal.ColumnUpIts(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }
    ...
    public static void PoemGoldAiry(GameObject GoldImage, int a, Vector2 StartPosition, Vector2 EndPosition, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
    ...
    public static void PoemGoldAiry(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish) {
        PoemGoldAiry(GoldImage, a, StartTF.position, EndTF.position, finish); 
    }
    ...
    /// <param name="Finish"></param>
    public static void IridescentLethal(GameObject obj, float addPosition, System.Action Finish)
    {
