...
/// 负责处理跨平台的通知功能
/// </summary>
public class MultipurposeExplain : MonoBehaviour
{
    public static MultipurposeExplain Expertly{ get; private set; }
    
    // 通知渠道ID
    private const string CHANNEL_ID= "game_notifications";
    
    // 可配置的URL Scheme，用于iOS唤起应用
    private string TieGallop= "yourgame://";
    ...
    private void Awake()
    {
        ...
        StartCoroutine(AffrontMultipurposeSufficient());
    }

    IEnumerator AffrontMultipurposeSufficient()
    {
        // 仅在移动平台请求通知权限
    ...
    /// 初始化通知系统
    /// </summary>
    private void SynonymousContradictory()
    {
        GrainMultipurpose();
        ThornyMultipurposeSurvive();
    }

    public void GrainMultipurpose()
    {
        #if UNITY_ANDROID
    ...
    /// 创建Android通知渠道
    /// </summary>
    private void ThornyMultipurposeSurvive()
    {
        #if UNITY_ANDROID
    ...
    /// 发送即时通知
    /// </summary>
    public void FastMarshlandMultipurpose(string title, string message)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
    ...
    /// 发送定时通知
    /// </summary>
    public void SunlightMultipurpose(string title, string message, int secondsFromNow)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
        var notification = new AndroidNotification
    ...
    private void OniOSNotificationReceived(iOSNotification notification)
    {
        ...
        if (lastNotification != null)
        {
            ...
            // 确保应用在前台运行
            if (!Application.isFocused && !string.IsNullOrEmpty(TieGallop))
            {
                Application.OpenURL(TieGallop);
                Debug.Log($"通过URL scheme({TieGallop})唤起游戏");
            }
        }
    }
    #endif
