...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].CoaxTug != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
...
#endif
        if (Resources.FindObjectsOfTypeAll<ADExplain>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADExplain>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADExplain>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].ScamCode != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
        if (GameObject.Find("Adjust").GetComponent<AdjustSdk.Adjust>().appToken != info.Adjust_APP_ID)
        {
            isSame = false;
            Debug.Log("adjust");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADExplain>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADExplain>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADExplain>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        
        UnityEditorInternal.ComponentUtility.CopyComponent(GameObject.Find("Adjust").GetComponent<AdjustSdk.Adjust>());
        ...
        adjustObj.GetComponent<AdjustSdk.Adjust>().appToken = info.Adjust_APP_ID;
        adjustObj.GetComponent<AdjustSdk.Adjust>().logLevel = AdjustSdk.AdjustLogLevel.Verbose;
        adjustObj.GetComponent<AdjustSdk.Adjust>().environment = AdjustSdk.AdjustEnvironment.Production;
        adjustObj.GetComponent<AdjustSdk.Adjust>().startManually = true;
        //adjustObj.GetComponent<AdjustSdk.Adjust>().eventBuffering = false;
        adjustObj.GetComponent<AdjustSdk.Adjust>().sendInBackground = false;
        adjustObj.GetComponent<AdjustSdk.Adjust>().launchDeferredDeeplink = true;
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<MarvelUserExplain>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<MarvelUserExplain>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<MarvelUserExplain>();
            }
        }
        GameObject.Find("MainManager").GetComponent<MarvelUserExplain>().OrchidID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<SoulOrExplain>())
        {
            GameObject.Find("MainManager").AddComponent<SoulOrExplain>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<SoulOrExplain>().Deity = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<SoulOrExplain>().appid = info.PackageName;
#endif

        Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].ScamCode = info.GameCode;
        Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].CoaxTug = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].CoaxLaterTug = info.BaseUrl + CStatus.LaterTug;
        Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].CoaxStatusTug = info.BaseUrl + CStatus.ConfigTug;
        Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].CoaxLoveTug = info.BaseUrl + CStatus.LoveTug;
        Resources.FindObjectsOfTypeAll<PinBeadEka>()[0].CoaxMarvelTug = info.BaseUrl + CStatus.MarvelTug;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CStatus.ConfigTug + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CStatus.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CStatus.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) => {
           ...
           if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("PinSlotExplain"))
               {
                    DestroyImmediate(GameObject.Find("PinSlotExplain"));
               }
           }
       },
       () => {
